...
Nuts.prototype.onElectronUpdater = function(req, res, next) {
    ...
    .then(function(latest) {
        ...
        .then(function() {
          ...
          if (contentType == 'exe' || contentType == 'zip') {
            var asset = _.find(latest.platforms, { filename: filename });
...
Nuts.prototype.onCrashReport = function(req, res, next) {
    ...
    form.parse(req, (err, fields, files) => {
      if (files.upload_file_minidump) {
        let file = files.upload_file_minidump[0]
        let email = fields.email ? fields.email[0] : 'Unknown (not logged in)'

        minidump.walkStack(file.path, (err, report) => {
          ...
          postmark.sendMail({
            ...
            headers: {
              References: fields.email ? `<electron/crash-reports/${email}@missiveapp.com>` : undefined
            },
            text: [
              `User: ${fields.email[0]}`,
              '',
              `${fields._productName[0]} (${fields._version[0]})`,
              `${fields.prod[0]} (${fields.ver[0]})`,
            ].join('\n'),
            attachments: [
              {
